...
const CartPage = () => {
  ...
  if (cart.length === 0) {
    return (
      <div className="container" style={{ padding: '6rem 0', textAlign: 'center' }}>
        <h2 style={{ fontSize: '2rem', marginBottom: '1rem' }}>{t('yourCartIsEmpty')}</h2>
        <Link to="/" style={{ color: 'var(--primary)', textDecoration: 'underline' }}>{t('continueShopping')}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="container page-enter" style={{ padding: '4rem 0' }}>
      <h2 style={{ fontSize: '2rem', marginBottom: '1.5rem' }}>{t('cart')} <span style={{ fontSize: '1rem', background: '#e5e7eb', padding: '2px 8px', borderRadius: '50%', color: '#333', verticalAlign: 'middle' }}>{cart.length}</span></h2>

      {/* Free Shipping Progress Bar */}
      <div style={{ marginBottom: '2.5rem', background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem', fontSize: '0.95rem', fontWeight: '500' }}>
          {isFreeShipping ? (
            <span style={{ color: '#10B981' }}>{t('freeShippingUnlocked')}</span>
          ) : (
            <span>{t('addMore')} {formatCurrency(remaining)} {t('moreForFree')}</span>
          )}
      ...
      <div className="cart-layout" style={{ alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          {cart.map(item => (
            <div key={item._id} className="cart-item" style={{ display: 'flex', gap: '2rem', alignItems: 'start', paddingBottom: '2rem', borderBottom: '1px solid var(--border-color)' }}>
              ...
              <div className="cart-item-details" style={{ flex: 1, display: 'flex', justifyContent: 'space-between' }}>
                <div>
        ...
        <div className="cart-summary" style={{ background: 'var(--bg-secondary)', borderRadius: '4px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <span style={{ color: 'var(--text-secondary)' }}>{t('estimatedTotal')}</span>
